...
pagar conns mvar manager appSettings pagamento = do
  ...
  logNow <- liftIO TIME.getCurrentTime
  liftIO $ pgLog conns appSettings "[Pagar] New payment" (Just $ correlationId pagamentoSync) Nothing Nothing (Just logNow)
  let (payId, planId) = head paymentRows
  logNow2 <- liftIO TIME.getCurrentTime
  liftIO $ pgLog conns appSettings ("[Pagar] New payment: ID " ++ show payId) (Just $ correlationId pagamentoSync) (Just planId) Nothing (Just logNow2)
  clientRes <- liftIO $ pagarPeloProcessor manager appSettings Default_ pagamentoSync
  _ <- (case clientRes of
             Left _ -> do
               liftIO $ (do
                 logNow3 <- TIME.getCurrentTime
                 pgLog conns appSettings ("[Pagar] First try fail: ID " ++ show payId) (Just $ correlationId pagamentoSync) (Just planId) Nothing (Just logNow3)
             ...
             Right _ -> do
               liftIO $ (do
                  logNow4 <- TIME.getCurrentTime
                  pgLog conns appSettings ("[Pagar] First try succeeded: ID " ++ show payId) (Just $ correlationId pagamentoSync) (Just planId) Nothing (Just logNow4)
